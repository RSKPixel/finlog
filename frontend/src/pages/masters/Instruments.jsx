...
const Instruments = () => {
  ...
  const handleSubmit = (e) => {
    ...
    fetch(`${api}/masters/instruments/save/`, {
      method: "POST",
      headers: {
  ...
  return (
    <div className="flex flex-col w-full items-center">
      <div className="flex flex-col w-2/4 items-center shadow-xl">
        ...
        <form autoComplete="off" className="grid grid-cols-2 gap-2 border bg-stone-900 rounded-b-sm w-full p-3 border-sky-900">
          <div className="flex flex-col gap-2">
            ...
            <input type="text" className="uppercase" name="instrument_name" value={formData.instrument_name} onChange={handleChange} />
          </div>
          <div className="flex flex-col gap-2">
      ...
      <InstrumentsSearch setFormData={setFormData} />
    </div>
  );
};

const InstrumentsSearch = ({ setFormData }) => {
  const { api, token, client } = useContext(AuthContext);
  ...
  return (
    <div className="flex flex-col w-2/4 items-center shadow-xl mt-4 text-sm">
      <div className="grid grid-cols-2 border w-full border-sky-900 p-1 rounded-t-sm bg-sky-950">
        <span>Instrument Name</span>
        <span>Instrument Group</span>
      </div>
      <div className="border bg-stone-900 p-1 w-full border-sky-900">
        {searchResults.map((result) => (
          <div key={result.instrument_id} className="hover:bg-sky-800 cursor-pointer p-1 grid grid-cols-2" onClick={() => setFormData(result)}>

...
const MutualFunds = () => {
    ...
    return (
        <div className="flex flex-col gap-4 w-full items-center">
                    ...
                    {assetClasses && (
                        <span className="flex flex-row gap-2 text-xs ms-2 text-gray-400">
                            ...
                            {assetClasses.map((ac, index) => (
                                <span key={index} className={`${assetClass == ac.asset_class && "font-bold text-white"} hover:underline underline-offset-4 cursor-pointer`} onClick={() => setAssetClass(ac.asset_class)}>
                                    {ac.asset_class} [{numeral(ac.percentage).format("0")}%]
                                </span>
                            ))}
                        </span>
                    )}
            ...
            {selectedFund && <FundDetails selectedFund={selectedFund} setSelectedFund={setSelectedFund} />}
            ...
            <div className="flex flex-col bg-neutral-900 border border-white/10 rounded-sm shadow-xl w-4/5 max-h-[100vh]">
                ...
                <div className="grid grid-cols-2 gap-3 p-4">
                        ...
                        {holdingSummary.nav_changes.map((row, index) => {
                            ...
                            return (
                                <div key={row.year} className="grid grid-cols-14 items-center p-2">
                                    ...
                                    <div className={`text-end ${rowsum >= 0 ? "text-green-500" : "text-red-500"}`}>{numeral(rowsum).format("0,0.00")}</div>
                                </div>
                            );
                        })}
                    </div>
                    ...
                    <div className="grid grid-cols-2 items-center gap-2 text-sm bg-neutral-800 border border-neutral-950 rounded-lg p-4 shadow-lg">
                        <div>Asset Class</div>
                        <select name="asset_class" onChange={handleChange} value={holdingSummary.holding.asset_class || ""}>
                            <option value="Debt">Debt</option>
                            <option value="Equity">Equity</option>
                            <option value="Gold">Gold</option>
                        </select>
                        <div>Goal Pot</div>
                        <select onChange={handleChange} name="goalpot" value={holdingSummary.holding.goalpot || ""}>
                    ...
                    <div className="text-xs text-end">Nav as on {moment(holdingSummary.holding.current_price_date).format("DD-MM-yyyy")}</div>
                </div>
            </div>
        </div>
    );

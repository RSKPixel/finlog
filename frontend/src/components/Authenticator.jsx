    ...
    if (loggedIn) {
        ...
        return (
            <div className='flex flex-col items-center w-full justify-center h-fit mt-4'>
                ...
                <h2 className='text-sm text-white/50'><i className='bi bi-lock-fill'></i> Please login to access your book</h2>
                <form autoComplete="off" className="flex flex-col gap-2 w-[350px]">
...
const Registration = ({ setRegistration }) => {
    ...
    const validation = () => {
        ...
        if (!formData.pan.trim()) { error.push('Fields marked * are mandatory') }
        if (!formData.name.trim()) { error.push('Fields marked * are mandatory') }
        if (!formData.email.trim()) { error.push('Fields marked * are mandatory') }
        if (!formData.password.trim()) { error.push('Fields marked * are mandatory') }
        if (!formData.confirmPassword.trim()) { error.push('Fields marked * are mandatory') }
        if (formData.password !== formData.confirmPassword) { error.push('Passwords do not match') }
    ...
    return (
        <div className='flex flex-col items-center w-full justify-center h-fit mt-4'>
            ...
            <form autoComplete="off" className="grid grid-cols-2 gap-4 w-[600px]">
